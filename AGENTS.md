...
- **Auth**: Bearer contra `env.PROXY_KEY`. NO usar `CUSTOM_API_KEY` (fue reemplazado).
...
- **Failover**: 429/503/timeout/400-context → próxima key del mismo provider (Gemini tiene GEMINI_API_KEY_2), luego siguiente modelo del pool.
...
- **Stats endpoint**: `GET /stats` autenticado con `PROXY_KEY` (misma key que chat).
...
```ts
{ id: "gemini-2.5-flash",          weight: 4, provider: "gemini",   envKey: "GEMINI_API_KEY_1" }
...
{ id: "deepseek-r1-distill-llama-70b", weight: 2, provider: "groq", envKey: "GROQ_API_KEY" }
```
